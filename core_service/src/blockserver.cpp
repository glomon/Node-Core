...
{
    if (ec.message() != "bad method") {
        DEBUG_COUT(what + ": " + ec.message());
    }
}

void do_session(beast::tcp_stream& stream, std::function<std::string(const std::string&, const std::string&, const std::string&, const std::string&)> processor, net::yield_context yield)
{
    ...
    while (keep_alive) {
        ...
        std::string request_url(req.target().data(), req.target().size());
        request_url.erase(std::remove(request_url.begin(), request_url.end(), '/'), request_url.end());
        
        std::string response_body = processor(request_body, request_url, req["Sign"].to_string(), req["PublicKey"].to_string());
...
void do_listen(net::io_context& io_context, unsigned short port, std::function<std::string(const std::string&, const std::string&, const std::string&, const std::string&)> processor, net::yield_context yield)
{
    beast::error_code ec;
